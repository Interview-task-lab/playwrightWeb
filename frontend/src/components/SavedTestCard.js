...
export function renderSavedTestCard(tc, { onRun, onView, onDelete }) {
  ...
  const deleteButton = tc.readOnly
    ? ''
      ...
      <div class="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div class="min-w-0">
          ...
          <div class="flex items-center gap-3 mt-1">
            ...
            <span class="lang-badge">${escapeHtml(tc.platform)}</span>
            <span class="text-xs text-surface-500">${stepsCount} steps</span>
            ${tc.readOnly ? `<span class="text-[10px] px-1.5 py-0.5 rounded bg-surface-800 text-surface-400 border border-surface-700/50 font-medium">Read-Only</span>` : ''}
            ${tc.domain_name ? `<span class="text-[10px] px-1.5 py-0.5 rounded bg-accent-500/10 text-accent-300 border border-accent-500/20 font-medium">${escapeHtml(tc.domain_name)}</span>` : ''}
          </div>
        </div>
      </div>
